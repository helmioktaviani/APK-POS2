                    ...
                    <small class="text-muted">{{ number_format($product->harga_jual) }}</small>
                        ...
                        <div class="col-3">
                            <input type="number" name="quantity" value="1" min="1"
                                    class="form-control {{ $sale->status === 'COMPLATED' ? 'readonly' : '' }}">
                        </div>

                        <div class="col-2">
                            <button class="btn btn-primary w-100 {{ $sale->status === 'COMPLATED' ? 'disabled' : '' }}">
                            +</button>
                        </div>
                    </form>
                    @endforeach
        ...
        <table class="table table-bordered mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @can('delete', $item)
                        <form method="POST" action="{{ route('itempenjualan.destory', $item->id }}">
                            @csrf @method('DELETE')
